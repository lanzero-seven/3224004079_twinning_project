...
def generate_exercises(n: int, r: int,
    ...
    while len(exercises) < n:
        ...
        if want_pure:
            ...
            #     continue
            #
            result=_generate_pure_integer_expression(operator_count, r)
        ...
        exercises.append(Exercise(root, answer))
        type_idx+=1

    return exercises


def _generate_pure_integer_expression(operator_count, r):
    if r < 2:
        return None

    if operator_count == 0:
        value=Fraction(random.randint(1, r - 1), 1)
        return ExpressionNode(value=value), value
    ...
    for _ in range(50):
        ...
        left_result=_generate_pure_integer_expression(left_count, r)
        right_result=_generate_pure_integer_expression(right_count, r)

        if left_result is None or right_result is None:
            continue

        left, lv=left_result
        right, rv=right_result
